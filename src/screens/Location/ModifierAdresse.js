...
import {UpdateAddress} from '../../api/UserLocation/Location'

class ModifierAdresse extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
         ...
         isSelected:false,
          ...
          ville:this.props.route.params.city,
          rue:this.props.route.params.rue,
          codepostale:this.props.route.params.codepostale,
          alias:this.props.route.params.alias,
          idadd:this.props.route.params.idadd
        };} 
...
render() {
  ...
  marginBottom: 5,}} onPress={()=>this._Adress(this.state.lan,this.state.long) }>
   ...
   <Button  buttonStyle={styles.acceptButton}
      title="Modifier" 
       onPress={()=>UpdateAddress(this.props.user.user.firstname,this.props.user.user.lastname,this.state.rue,this.state.codepostale,this.state.ville,this.props.user.user.id,this.state.alias,this.state.idadd)}  ></Button>
...
export default  connect(mapStateToProps, mapDispatchToProps)(ModifierAdresse);